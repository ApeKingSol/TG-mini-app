...
/** One entry per trade-in tier. Every image is now a transparent-background .webp, cropped
 * and rescaled to a shared 1024x540 canvas with each car's own width normalized to the same
 * fraction of frame and bottom-aligned to the same ground line (see the one-off normalization
 * pass — the 15 newer renders arrived as black-background JPEGs at wildly different in-frame
 * scales, some over 2x the apparent size of others). */
